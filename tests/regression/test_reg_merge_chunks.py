...
class TestPerChunkTokenBudgetInvariant:
    ...
    def test_fractional_ratio_keeps_single_word_documents(self):
        assert merge_chunks(["one", "two"], target_size=20, word_token_ratio=0.75) == [
            "one two"
        ]
    ...
    def test_overlap_is_clipped_when_it_cannot_fit_the_budget(self):
        chunks = merge_chunks(
            [" ".join(f"w{i}" for i in range(30))],
            target_size=20,
            overlap=10,
            word_token_ratio=3.0,
        )

        assert _within_budget(chunks, 20, 3.0)
        assert chunks[1].split() == ["w1", "w2", "w3", "w4", "w5", "w6"]

    @pytest.mark.parametrize(("target", "ratio"), [(1, 1.0), (2, 2.0)])
    def test_overlap_is_omitted_when_one_token_fills_the_budget(self, target, ratio):
        assert merge_chunks(
            ["a b c"], target_size=target, overlap=1, word_token_ratio=ratio
        ) == ["a", "b", "c"]

    def test_token_larger_than_target_still_forms_its_own_chunk(self):
        assert merge_chunks(
            ["a b c"], target_size=1, overlap=1, word_token_ratio=2.0
        ) == ["a", "b", "c"]
