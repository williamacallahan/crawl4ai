...
- **Stale-lease backstop**: `active_requests` is only trusted while the browser was
  ...
  hung or leaked, so the browser is retired, its outstanding admission permits
  are returned exactly once, and it is force-closed instead of pinned forever.
- **Background close**: `close()` on a wedged browser can hang, so the janitor and
  janitor, monitor actions, replacement, dedicated release, and `close_all()`
  detach pool state under `LOCK`, then wait outside it. Closes have a 60s cap,
  after which the browser's Playwright driver is stopped so its Chromium process
  group cannot survive unreferenced. In-flight closes still count toward
  `max_browser_instances`, and `close_all()` drains them before shutdown.
