...
previously accepted a bare-dict value for three allowlisted nested-object
fields of `CrawlerRunConfig` (`geolocation`, `table_extraction`,
`scraping_strategy`). `CrawlerRunConfig.__init__` stored the bare dict
...
These tests lock in the fixed behavior:
    - a bare dict for any of the three fields is rejected at the gate with
...
class TestBareDictGateAtLibraryLevel:
    ...
    def test_bare_dict_does_not_reach_constructor(self):
        ...
        for field in ("geolocation", "table_extraction", "scraping_strategy"):
