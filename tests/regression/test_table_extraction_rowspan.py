...
def test_rowspan_stops_at_each_explicit_row_group_boundary():
    table = (
        "<table><thead><tr><th>Left</th><th>Right</th></tr></thead>"
        '<tbody><tr><td rowspan="2">A</td><td>B</td></tr></tbody>'
        '<tbody><tr><td rowspan="2">C</td><td>D</td></tr></tbody>'
        "<tfoot><tr><td>E</td><td>F</td></tr></tfoot></table>"
    )
    _, data = _extract(table)

    assert data["rows"] == [["A", "B"], ["C", "D"], ["E", "F"]]


def test_rowspan_stops_at_thead_boundary_between_direct_rows():
    table = (
...
def test_rowspan_zero_extends_through_only_the_current_row_group():
    table = (
        "<table><thead><tr><th>Left</th><th>Right</th></tr></thead><tbody>"
        '<tr><td rowspan="0">A</td><td>B</td></tr>'
        "<tr><td>C</td></tr><tr><td>D</td></tr></tbody>"
        "<tbody><tr><td>E</td><td>F</td></tr></tbody></table>"
    )
    _, data = _extract(table)

    assert data["rows"] == [["A", "B"], ["A", "C"], ["A", "D"], ["E", "F"]]


def test_invalid_rowspan_defaults_to_one_without_discarding_table():
    for rowspan in ("", "bad", "-2"):
        table = (
            "<table><caption>Values</caption><thead>"
            "<tr><th>Left</th><th>Right</th></tr></thead><tbody>"
            f'<tr><td rowspan="{rowspan}">A</td><td>B</td></tr>'
            "<tr><td>C</td><td>D</td></tr></tbody></table>"
        )

        tables = _extract_all(table)
...
def test_empty_continuation_row_consumes_pending_rowspan():
    table = (
        "<table><thead><tr><th>Left</th><th>Right</th></tr></thead><tbody>"
    ...
    assert data["rows"] == [["A", "B"], ["", "B"], ["C", "D"]]


def test_nested_subtable_does_not_create_outer_rowspan_rows():
    table = (
        "<table><caption>Outer report</caption><thead>"
        "<tr><th>Group</th><th>Value</th></tr></thead><tbody>"
        '<tr><td rowspan="2">A</td><td><table><tr>'
        "<td>nested-one</td><td>nested-two</td>"
        "</tr></table>outer-one</td></tr>"
        "<tr><td>outer-two</td></tr>"
        "</tbody></table>"
    )
    _, data = _extract(table)

    assert data["rows"] == [["A", "outer-one"], ["A", "outer-two"]]
...
def test_colspan_uses_html_limits_in_headers_and_body():
    for value, expected in [("", 1), ("bad", 1), ("-2", 1), ("0", 1), ("2junk", 2), ("1001", 1000), ("999999999", 1000)]:
        for use_thead in (False, True):
            header = f'<tr><th colspan="{value}">Value</th></tr>'
            if use_thead:
                header = "<thead>" + header + "</thead>"
            _, data = _extract(
                f'<table>{header}<tr><td colspan="{value}">A</td></tr></table>',
                table_score_threshold=0,
            )
            assert len(data["headers"]) == expected
            assert data["rows"] == [["A"] * expected]


def test_long_numeric_span_is_bounded_before_integer_conversion():
    value = "9" * 5000
    _, data = _extract(
        f'<table><tr><td colspan="{value}">A</td></tr></table>',
        table_score_threshold=0,
    )
    assert data["rows"] == [["A"] * 1000]
