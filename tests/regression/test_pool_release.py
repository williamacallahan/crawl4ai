...
class _FakeStrategy:
    def __init__(self):
        self.browser_manager = type(
            "_FakeManager", (), {"browser": _FakeBrowser(), "default_context": None}
        )()
...
class FakeCrawler:
    ...
    The pool touches .active_requests and probes liveness through
    crawler_strategy.browser_manager (see crawler_pool._is_live), so the
    stand-in has to present a connected browser or every pool hit is
    discarded as dead.
    """
    ...
    async def close(self):
        self.closed = True
        self.crawler_strategy.browser_manager.browser.connected = False
...
class TestReleaseNeverSuspends:
    """The measured production leak: release_crawler used to take the pool LOCK.

    On a client disconnect Starlette cancels the response scope, and anyio
    re-delivers that cancellation at every subsequent suspension point. Any
    await inside release_crawler is therefore skipped whenever the lock is
    contended, leaving active_requests permanently above zero and the browser
    pinned against the janitor for the container's life.
    """
    ...
    def test_release_completes_without_yielding_to_the_event_loop(self):
        ...
        try:
            ...
            raise AssertionError(
                "release_crawler suspended; a cancelled request will skip the decrement"
            )
...
class TestJanitorSurvivesABadPass:
    """A raise inside the loop used to end reclamation for the container's life.

    The task handle is retained on app.state, so the exception is parked until
    shutdown and never printed: the pool stops being reclaimed while the
    container keeps reporting healthy.
    """

    @pytest.mark.asyncio
    async def test_a_failing_pass_does_not_kill_the_loop(self, monkeypatch):
        calls = []
        ...
        async def failing_then_ok():
            calls.append(1)
            # Yield, or the guarded loop spins without ever reaching the poller.
            await real_sleep(0)
            if len(calls) == 1:
                raise RuntimeError("boom")

        monkeypatch.setattr(crawler_pool, "_janitor_pass", failing_then_ok)
        monkeypatch.setattr(
            crawler_pool.asyncio, "sleep", lambda _s: real_sleep(0)
        )
        crawler_pool.LAST_JANITOR_PASS = 0.0

        task = asyncio.create_task(crawler_pool.janitor())
        for _ in range(50):
            await asyncio.sleep(0)
            if crawler_pool.LAST_JANITOR_PASS:
                break
        task.cancel()
        with pytest.raises(asyncio.CancelledError):
            await task

        assert len(calls) >= 2, "loop stopped after the failing pass"
        assert crawler_pool.LAST_JANITOR_PASS > 0, "heartbeat never advanced"
