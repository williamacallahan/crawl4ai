...
def test_short_corpus_all_above_threshold_not_halved(monkeypatch):
    """Sub-``at_least_k`` short corpus where *every* chunk clears the threshold:
    the shrinkage clause lowers the floor to ``len(documents)//2`` but no chunk
    is dropped (previously 12 -> 6, 50% loss)."""
    n = 12
    strategy, documents = _build_strategy(
        monkeypatch, sim_threshold=0.3, target_sims=[0.99] * n
    )
