...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("https://[fe80::1%25eth0]/", "fe80::1%eth0", 443),
        (
...
def test_from_url_passes_ipv6_url_authority_to_socket(
    url, expected_host, expected_port
):
    ...
    assert args[0] == (expected_host, expected_port)
...
def test_scoped_ipv6_keeps_interface_out_of_tls_identity():
    with patch("crawl4ai.ssl_certificate.socket.create_connection") as connect, patch(
        "crawl4ai.ssl_certificate.ssl.create_default_context"
    ) as context:
        context.return_value.wrap_socket.return_value.__enter__.return_value.getpeercert.return_value = None
        SSLCertificate.from_url("https://[fe80::1%25eth0]:8443/")

    connect.assert_called_once_with(("fe80::1%eth0", 8443), timeout=10)
    context.return_value.wrap_socket.assert_called_once_with(
        connect.return_value.__enter__.return_value, server_hostname="fe80::1"
    )
