...
class TestCompose:
    ...
    def test_redis_state_is_a_named_volume_not_tmpfs(self, compose):
        """This fork keeps Redis state DURABLE, diverging from upstream here.

        Upstream mounts /var/lib/redis as tmpfs, which discards every queued and
        leased job on each container recreate. /crawl/job is a durable contract
        backed by a Redis Stream that supervised workers consume, so the state
        must outlive the container. Ownership still lands on appuser: Dockerfile
        creates /var/lib/redis as appuser:appuser and Docker seeds a new named
        volume from the image, ownership included. Every other hardening
        assertion in this class still applies.
        """
        assert "/var/lib/redis:uid=999,gid=999,mode=0700" not in compose
        assert "crawl4ai-redis:/var/lib/redis" in compose
