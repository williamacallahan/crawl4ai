...
@pytest.mark.asyncio
async def test_real_browser_recycle_serves_next_page_after_restart():
    """After an in-place recycle the restarted browser serves a fresh page.

    This corroborates the fix end-to-end: with the pool treating the recycling
    browser as live, the library's own back-pressure (``_admit_page_acquisition``
    awaiting ``_recycle_done``) releases the next ``get_page`` only after the
    restart, and that page comes from the restarted browser (a different object
    from the pre-recycle page).
    """
    config = BrowserConfig(
        headless=True,
        max_pages_before_recycle=1,
    ...
    try:
        ...
        assert first_browser is not None and first_browser.is_connected()

        first_page, _ = await manager.get_page(CrawlerRunConfig())
        await manager.release_page_with_context(first_page)

        crawler = _crawler(manager)
        await _sample_until_recycle_done(manager, crawler)
        if manager._recycle_task is not None:
            await manager._recycle_task
